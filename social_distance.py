...
def draw_detection_box(frame,x1,y1,x2,y2,color):
    ...
    cv2.rectangle(frame,(x1,y1),(x2,y2), color, 2) #x1 and y1 are top left, x2 and y2 are bottom right
...
def main():
    ...
    while True:
        ...
        # Detecting objects
        blob = cv2.dnn.blobFromImage(frame_resized, 1/255, (416, 416), (0, 0, 0), True, crop=False)  #********Scaling pixel value to 0-1
        ...
        for out in outs:
            for detection in out:
                ...
                #if prediction is 50% and class id is 0 which is 'person' (object is detected)
                if confidence > 0.5 and class_id == 0:
                    ...
                    # Rectangle coordinates    #top left coordinates
                    x = int(center_x - w / 2)
                    y = int(center_y - h / 2)
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                for k in range (len(centroids)):
                    ...
                    if get_distance(c[0],centroid[0],c[1],centroid[1]) <=69:
